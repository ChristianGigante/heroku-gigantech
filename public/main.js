        ...
        socket.on('chat message', function (msg) {
          ...
          if (msg.split(": ")[0] == id && !msg.includes("NewUserLogIn")) {
            if (!msg.includes("TYP1NG@M3$$@G3")) {
              $('#messages').append("<div style='text-align:right;'><h2 id='me' class='btn btn-outline-success disabled'>" + msg.split(": ")[2] + "</h2></div>");
            }
          } else {
            if (msg.includes("NewUserLogIn")) {
              ...
              $("#typing").remove();
              $('#messages').append("<div><h2 id='other' class='btn btn-outline-primary disabled'>" + msg.split(": ")[1] + ": " + msg.split(": ")[2] + "</h2></div>");
            }
          }
